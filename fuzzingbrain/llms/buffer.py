...
class LLMCallBuffer:
    ...
    async def start(self):
        ...
        logger.info("LLMCallBuffer started")
    ...
    async def _flush_batch(self) -> int:
        """
        Flush a batch of records from Redis to MongoDB.
                ...
                if self._redis:
                    ...
                    with self._memory_lock:
                        if not self._memory_buffer:
                            return 0
                        docs = self._memory_buffer[: self.flush_batch_size]
                        self._memory_buffer = self._memory_buffer[self.flush_batch_size :]
